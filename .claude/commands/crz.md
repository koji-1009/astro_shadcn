...
Islands in layers 3-4 should be wrapped in an Error Boundary (React) or equivalent. If the island crashes, display a fallback UI instead of a blank space. This structurally enforces the isolation guarantee — the rest of the page remains intact.
...
* Static configuration (build-time constants, `as const` objects) may be imported directly in islands — no need to serialize through props
